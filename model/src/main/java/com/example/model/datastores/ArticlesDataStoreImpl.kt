...
import com.example.model.controllers.ArticleParserImpl
...
class ArticlesDataStoreImpl @Inject constructor() : ArticlesDataStore {
  ...
  override fun getFavoriteArticles(): Single<List<FavoriteArticle>> {
    return articleDao.getFavoriteArticles()
      ...
      .map { item -> transformer.toFavoriteArticle(item) }
      .toList()
  }
  ...
  override fun getArticle(id: Int): Single<Article> {
    return articleDao.getArticle(id).map { item -> transformer.toArticle(item) }
  }

  override fun getLastAddedArticles(count: Int): Single<List<LastAddedArticle>> {
    return articleDao.getLastAddedArticles()
      .flatMapObservable { list -> Observable.fromIterable(list) }
      .map { item -> transformer.toLastAddedArticle(item) }
      .toList()
  }
  ...
  override fun addArticle(url: String, categroy: String): Completable {
    ...
    return Single.just(
      ...
      // TODO
      var path = downloadController.providePath("example")
      var articleParser = ArticleParserImpl(article, path)
      var article = articleParser.parse()
      val tasks = articleParser.downloadTaskList
      downloadController.load(tasks)
    }.flatMapCompletable { article -> articleDao.addArticle(transformer.toArticleEntity(article)) }
  }
}